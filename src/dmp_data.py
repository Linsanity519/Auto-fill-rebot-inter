...
新增文件，只被 DmpRunner 的 id_list 模式用到 —— 老的 datasource / wizard_data
一行不动。

清单支持 .xlsx / .xlsm / .csv，列名认这几种写法：
  人群ID   ← 人群id / ID / id / 人群编号 / crowd_id
  延期至   ← 有效期至 / 人群有效期至 / 延期日期 / 目标日期（留空 = 延到系统最大日期）
  人群名称 ← 名称（选填，只用于核对，不参与定位）
"""
...
ID_ALIASES = ("人群id", "人群ID", "id", "ID", "人群编号", "crowd_id", "crowdid")
DATE_ALIASES = ("延期至", "有效期至", "人群有效期至", "延期日期", "目标日期", "延长至")
NAME_ALIASES = ("人群名称", "名称", "人群名")


class DmpDataError(Exception):
    pass
...
def _clean_id(value) -> str:
    """Excel 会把纯数字 ID 读成 35697.0，这里统一成字符串。"""
    if value is None:
        return ""
...
def load(path: str) -> list[dict]:
    ...
    「这个 ID 在页面上有没有」要连页面才知道，放在 DmpRunner.preview 里查。
    """
    p = Path(path)
    if not p.exists():
        raise DmpDataError(f"人群清单文件不存在：{p}")
    if p.suffix.lower() not in (".xlsx", ".xlsm", ".csv"):
        raise DmpDataError(f"人群清单只认 .xlsx / .xlsm / .csv，当前是：{p.suffix}")

    rows = _read_rows(p)
    if not rows:
        raise DmpDataError(f"{p.name} 里没有数据行")

    headers = [h for h in rows[0] if h != "_row"]
    id_col = _pick(headers, ID_ALIASES)
    if not id_col:
        raise DmpDataError(
            f"{p.name} 里找不到「人群ID」列（表头现在是：{headers}）。"
    ...
    for rec in rows:
        ...
        issues = []
        if not cid:
            issues.append(f"第{row_no}行：人群ID 为空")
        elif cid in seen:
            issues.append(f"第{row_no}行：人群ID「{cid}」和第{seen[cid]}行重复")
        else:
            seen[cid] = row_no

        d = parse_date(raw)
        if raw_s and d is None:
            issues.append(f"第{row_no}行：延期至「{raw_s}」认不出来，请写成 2026-08-20 这种格式")
