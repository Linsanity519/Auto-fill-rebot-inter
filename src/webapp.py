...
class Api:
    ...
    def usage_summary(self) -> dict:
        ...
        ⚠ 只读本地：全团队那份是随包 / 运行时拉下来的快照（usage.fetch_team），
          不在进首页这条路上现算。
        """
        try:
            # 「显示的是本机还是全团队」这个判断只有一处实现，在 usage.team_view。
            # ⚠ 别在这儿再拆字段：以前就是在这里一项一项搬，搬漏了 week/weeks/recent，
            #   于是同一张卡上一半团队一半本机，谁都看不出来。
            out = usage.team_view(usage.summarize(self.settings), usage.load_team())
