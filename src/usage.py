...
# ⚠ 1.1.14 之前这里还有一整套「每周累计 + 上报记账」：weekly_buckets / report_rows /
#   mark_reported / _row_sign / usage-reported.json …… 回传改成「一次运行一条」之后
#   全部没用了，1.1.16 删掉（要看就翻 git 历史）。现在的分工是：
#     · 本机：usage.jsonl 一次运行一条 → report 的发件箱 → 表格 / 群
#     · 收集端：tools/collect_usage.py 读表算 team.json（口径只在那里算一次）
#   下面 parse_report 是**老路子专用**：还没升级的人仍在往群里发周累计消息，
#   collect_usage --clipboard 收它们时要用。等大家都升上来了，它也可以删。
