...
安装包装到 `%LOCALAPPDATA%\配置助手`，是个全新的空目录；老同事手上那个解压出来的
`配置助手分发包_vX.Y.Z\` 文件夹不会被自动读取。**装完需要手动把这几样拷过去**，
否则他们打开新版会发现策略中心是空的、首页统计从零开始：

| 从旧文件夹 | 拷到 `%LOCALAPPDATA%\配置助手\` | 不拷的后果 |
|---|---|---|
| `config\strategies\` | 同名目录 | 策略中心配置全没了 |
| `config\prep\` | 同名目录 | 原生商广的准备参数要重填 |
| `data\` | 同名目录 | 自己的 Excel 不在了 |
| `output\usage.jsonl` | 同名文件 | 首页「我的统计」从零开始 |

`config\settings.yaml` 不用拷（新版多了 `update:` 段）。真拷了也不会坏 ——
`src/settings.py` 会用 `assets/settings.default.yaml` 把缺的字段补齐。

浏览器登录态（`.chrome-profile`）也不拷，重新登录一次即可。
